...
<script type="text/javascript">
    ...
    // 设置数据源URL
    dt.setDataSource("<?= site_url('/admin/role/query') ?>");
    ...
    dt.setRowDblClickEvent(function(data) {
        //PopWindow( { id: 'manager', title: '修改用户', url: '<?= site_url('/role/editform') ?>/'+data.id, width: 300, height: 250 } )
        $(this).popModal({
            url: '<?= site_url('/admin/role/editform') ?>/' + data.id,
            title: '修改用户'
        })
    ...
    // 添加表格字段
    dt.addColumn("name", "角色名", 100);
    dt.addColumn("desc", "说明");
    dt.addColumn("modified", "修改时间", 150, secondFieldToDateTime);
    ...
    function deleteManager(id)
    {
        ...
        $.post('<?= site_url('/admin/role/delete') ?>/' + id, function(response) {
...
<div id="container" class="container" style="min-width: 940px">
    <form class="form-inline" id="queryForm" action="" method="post">
        <label>角色名：<input type="text" class="input-medium" name="search[name:like]" /></label>
        <button type="submit" class='btn'><i class="icon-search"></i> 查询</button>
        <div class="btn-group pull-right">
            <a class="btn" title="添加角色" href="/admin/role/addform" data-toggle="popmodal" data-destroy="0"><i class="icon-plus"></i><span class="text">添加</span></a>
        </div>
    </form>
